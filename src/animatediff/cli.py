...
def generate(
    ...
    context: Annotated[
        ...
        typer.Option(
            ...
            max=32,
            help=(
                "Number of frames to condition on (default: max of <length> or 32). max for motion module v1"
                " is 24"
            ),
            show_default=False,
            rich_help_panel="Generation",
    ...
    if use_controlnet_tile == False:
        if use_controlnet_line_anime == False:
            if use_controlnet_ip2p == False:
                raise ValueError(
                    f"At least one of them should be enabled. {use_controlnet_tile=},"
                    f" {use_controlnet_line_anime=}, {use_controlnet_ip2p=}"
                )
...
def refine(
    ...
    interpolation_multiplier: Annotated[
        ...
        typer.Option(
            ...
            max=10,
            help=(
                "Interpolate with RIFE before generation. (I'll leave it as is, but I think interpolation"
                " after generation is sufficient)."
            ),
            rich_help_panel="Generation",
        ),
    ] = 1,
