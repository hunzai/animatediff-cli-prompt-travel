...
class Animate:
  def __init__(self, config_json_path=None):
    ...
    self.controlnet_images_path = os.path.join(
        self.REPO_PATH,
        "data",
        "controlnet_image"
    )
  ...
  def download_models(self):
      ...
      #
      MODEL_REPO_PATH= os.path.join(
          self.REPO_PATH_PARENT, "models"
      )
      ...
      if not os.path.exists(PATH_DWPose) or not os.path.isfile(PATH_DWPose_DW11):
        subprocess.run([
            "aria2c", "--console-log-level=error", "-c", "-x", "16", "-k", "1M", "https://huggingface.co/yzd-v/DWPose/blob/main/dw-ll_ucoco_384.onnx", "-d", str(PATH_DWPose), "-o", "dw-ll_ucoco_384.onnx"
        ])
      ...
      if not os.path.exists(PATH_SD) or not os.path.isfile(PATH_SD_MISTOON):
        subprocess.run([
            "aria2c", "--console-log-level=error", "-c", "-x", "16", "-k", "1M", "https://civitai.com/api/download/models/108545", "-d", str(PATH_SD), "-o", "mistoonAnime_v20.safetensors"
        ])
      ...
      if not os.path.exists(PATH_SD) or not os.path.isfile(PATH_SD_DREAM):
        subprocess.run([
            "aria2c", "--console-log-level=error", "-c", "-x", "16", "-k", "1M", "https://civitai.com/api/download/models/128713", "-d", str(PATH_SD), "-o", "dreamshaper.safetensors"
        ])
  ...
  def update_config_set_ytframes(self, downloaded_video_name, video_frames_dir):
      ...
      for cntrl in var_dict:
        if var_dict[cntrl]:
          cntrl_dir_path = os.path.join(
            controlnet_image_path_video,
            cntrl
          )
